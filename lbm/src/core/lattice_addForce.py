...
from   lbm.src.core.nb_addForce       import *
...
class lattice:
    ...
    def __init__(self, app):
        ...
        self.results_dir = './knudsen_results/'
    ...
    ### Computer Force
    def cacu_force(self):
        self.c_rho[:,:]  = self.const*self.rho1[:,:]
        # self.shi[:,:]    = self.rho[:,:]*self.R*self.T/(1.0-self.const*self.rho[:,:]) - self.a*self.alpha*self.rho[:,:]**2/(1.0+2.0*self.c_rho[:,:]-self.c_rho[:,:]**2)
        self.shi[:,:]    = self.rho1[:,:]*self.R*self.T/(1.0-self.const*self.rho1[:,:]) - self.a*self.alpha*self.rho1[:,:]**2/(1.0+2.0*self.c_rho[:,:]-self.c_rho[:,:]**2)
    ...
    def zou_he_top_wall_velocity(self):
        ...
        nb_slip_top_wall_velocity(self.lx, self.ly, self.g,
                                  self.N, self.k_n, self.fi_kn, 
                                  self.modify, self.b_rho)
    ...
    def zou_he_bottom_wall_velocity(self):
        ...
        nb_slip_bottom_wall_velocity(self.lx, self.ly, self.g,
                                  self.N, self.k_n, self.fi_kn, 
                                  self.modify, self.b_rho)
    ...
    def generate_image(self, obstacles):
        ...
        fig, ax = plt.subplots(figsize=(image_width/ 5, image_height/ 5))
    ...
    def add_obstacle_1(self, filename):
        obs = np.empty((0,2), dtype=int)
        bnd = np.empty((0,3), dtype=int)
        ibb = np.empty((0),   dtype=float)
