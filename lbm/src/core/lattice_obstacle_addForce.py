...
from   lbm.src.core.nb_obstacle_addForce       import *
...
class lattice:
    ...
    def __init__(self, app):
        ...
        if hasattr(app, "Cs_real"): self.Cs_real = app.Cs_real
        if hasattr(app, "rho_real"): self.rho_real = app.rho_real
        if hasattr(app, "Crho"): self.Crho = app.Crho
        if hasattr(app, "r"): self.r = app.r

        ################### added
        if hasattr(app, "k_n"):       self.k_n       = app.k_n
        if hasattr(app, "G"):         self.G         = app.G
        if hasattr(app, "const"):     self.const     = app.const
        if hasattr(app, "a"):         self.a         = app.a
        if hasattr(app, "alpha"):     self.alpha     = app.alpha
        if hasattr(app, "R"):         self.R         = app.R
        if hasattr(app, "T"):         self.T         = app.T
        if hasattr(app, "N"):         self.N         = app.N
        if hasattr(app, "fi_kn"):     self.fi_kn     = app.fi_kn
        if hasattr(app, "modify"):    self.modify    = app.modify
        if hasattr(app, "b_rho"):     self.b_rho     = app.b_rho
        ####################

        # Output dirs
        time             = datetime.now().strftime('%Y-%m-%d_%H_%M_%S')
        self.results_dir = './porous_kn_results/'
        ...
        print('# dt         = '+'{:f}'.format(self.dt*1e6))
        print('# dx         = '+'{:f}'.format(self.dx))
        print('# nx         = '+str(self.nx))
        print('# ny         = '+str(self.ny))
        print('')
        print('# Modify     = '+str(self.modify))
        print('# Kn         = '+str(self.k_n))
        print('# b_rho      = '+str(self.b_rho))
        print('# fi_Kn      = '+str(self.fi_kn))
        print('# r          = '+str(self.r))
        print('')
    ...
    def set_default_lbm(self):
        ...
        # Density arrays
        self.g       = np.ones((self.q,  self.nx, self.ny))
        self.g_eq    = np.ones((self.q,  self.nx, self.ny))
        self.g_up    = np.ones((self.q,  self.nx, self.ny))
        ...
        self.rho_left   = np.zeros(    self.ny)
        self.p_left     = np.zeros((   self.ny))
        ...
        self.Force   = np.zeros((2, self.nx, self.ny)) # add force term

        # Force array
        self.Fi      = np.zeros((self.q,  self.nx, self.ny))
        self.c_rho   = np.zeros((   self.nx, self.ny))
        self.shi     = np.zeros((   self.nx, self.ny))
        self.qiuhe   = np.zeros((2, self.nx, self.ny))
        self.rho1    = np.ones ((   self.nx, self.ny))

    ### ************************************************
    ### Computer Force
    def cacu_force(self):
        self.c_rho  = self.const*self.rho_lbm
        # self.shi[:,:]    = self.rho[:,:]*self.R*self.T/(1.0-self.const*self.rho[:,:]) - self.a*self.alpha*self.rho[:,:]**2/(1.0+2.0*self.c_rho[:,:]-self.c_rho[:,:]**2)
        self.shi[:,:]    = self.rho_lbm*self.R*self.T/(1.0-self.const*self.rho_lbm) - self.a*self.alpha*self.rho_lbm**2/(1.0+2.0*self.c_rho-self.c_rho**2)
        
        for i in range(self.lx):
            for j in range(self.ly):
                self.qiuhe[:,i,j]  = (self.w[0]*self.shi[i,j]*self.c[0] + self.w[1]*self.shi[i+1,j]*self.c[1] + self.w[2]*self.shi[i-1,j]*self.c[2]
                                    + self.w[3]*self.shi[i,j+1]*self.c[3] + self.w[4]*self.shi[i,j-1]*self.c[4] + self.w[5]*self.shi[i+1,j+1]*self.c[5]
                                    + self.w[6]*self.shi[i-1,j-1]*self.c[6] + self.w[7]*self.shi[i-1,j+1]*self.c[7] + self.w[8]*self.shi[i+1,j-1]*self.c[8])
                                     
        self.Force[:,:,:]  = -self.G*self.shi[:,:]*self.qiuhe[:,:,:]
        

        for i in range(self.lx):
            for j in range(self.ly):
                for q in range(8):
                    #self.Fi[q,i,j] = self.w[q]*(1.0-1.0/(2.0*self.tau_lbm))*((self.c[q]-self.u[:,i,j])/(self.Cs**2)+self.c[q]*(self.c[q].dot(self.u[:,i,j]))/(self.Cs**4)).dot(self.Force[:,i,j])
                    self.Fi[q,i,j] = self.w[q]*(1.0-1.0/(2.0*self.tau_lbm))*(self.c[q].dot(self.Force[:,i,j]))/(self.Cs**2)
    ...
    def macro(self):
        ...
        self.u[0,:,:] = np.tensordot(self.c[:,0],
                                     self.g[:,:,:],
                                     axes=(0,0))/self.rho[:,:] + self.Force[0,:,:]/(2.0*self.rho[:,:])
        self.u[1,:,:] = np.tensordot(self.c[:,1],
                                     self.g[:,:,:],
                                     axes=(0,0))/self.rho[:,:] + self.Force[1,:,:]/(2.0*self.rho[:,:])
    ...
    def cacu_k(self, it):
        ...
        if det_rho ==0:
            ...
            k = out_flux / self.ly * self.lx * self.nu_lbm / (3.0*det_rho)
    ...
    def collision_stream(self):
        ...
        nb_col_str(self.g, self.g_eq, self.g_up,
                   self.om_p_lbm, self.om_m_lbm, self.om_lbm,
                   self.c, self.ns,
                   self.nx, self.ny,
                   self.lx, self.ly, self.Fi)
    ...
    ### ************************************************
    ### Obstacle halfway bounce-back no-slip b.c. by ww
    def bounce_back_obstacle_ww(self, obstacle):

        nb_bounce_back_obstacle_ww(obstacle.boundary,
                                self.ns, self.c,
                                self.g_up, self.g, self.u, self.lattice)
    ...
    def bounce_back_obstacle_kn_ww(self, obstacle):
        ...
        nb_bounce_back_obstacle_kn_ww(obstacle.boundary,
                                 self.g, self.g_up, self.r)

    ### ************************************************
    ### Zou-He left wall velocity b.c.
    def zou_he_left_wall_velocity(self):

        # nb_zou_he_left_wall_velocity_new(self.lx, self.ly, self.u,
        #                              self.u_left, self.rho, self.g, self.lattice)
        nb_zou_he_left_wall_velocity(self.lx, self.ly, self.u,
                                     self.u_left, self.rho, self.g)

    ### ************************************************
    ### Zou-He right wall pressure b.c.
    def zou_he_left_wall_pressure(self):

        nb_zou_he_left_wall_pressure(self.lx, self.ly, self.u,
                                      self.rho_left, self.u_left,
                                      self.rho, self.g)
    ...
    def periodic_left(self):
        ...
        nb_periodic_left(self.lx, self.ly, self.u,
                                               self.rho, self.g)
    ...
    def periodic_right(self):
        ...
        nb_periodic_right(self.lx, self.ly, self.u,
                                               self.rho, self.g)
    ...
    def generate_image(self, obstacles):
        print('fig file start')

        # Add obstacle border
        lat = self.lattice.copy()
        print('fig file copy Finished')
        ...
        image_height, image_width = np.rot90(lat).shape
        print('fig file load Finished')
        fig, ax = plt.subplots(figsize=(image_width/ 50, image_height/ 50))
        lat = np.ma.masked_where((lat > 0.0), lat)
        ax.imshow(np.rot90(lat),
                    vmin= -1.0,
        ...
        # ax.set_xticks(np.arange(0.5, self.nx, 1.0))
        # ax.set_yticks(np.arange(0.5, self.ny, 1.0))
        plt.axis('off')
    ...
    def add_obstacle_1(self, in_dat):
        obs = np.empty((0,2), dtype=int)
        bnd = np.empty((0,3), dtype=int)
