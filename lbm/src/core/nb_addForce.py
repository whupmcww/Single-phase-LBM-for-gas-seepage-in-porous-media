...
@njit(parallel=True,cache=True,fastmath=True,nogil=True)
def nb_col_str(g, g_eq, g_up, om_p, om_m, c, ns, nx, ny, lx, ly, f):
...
### Slip top wall velocity b.c.
@njit(cache=True,fastmath=True,nogil=True)
def nb_slip_top_wall_velocity(lx, ly, g, N, k_n, fi_kn, modify, b_rho):
    first = (1.0/N)**2/(4.0*k_n*fi_kn)
    sigma = 1.0
    sigma_v = (2.0-sigma)/sigma
    B1 = (1.0-0.1817*sigma)
    second = B1*sigma_v
    B2 = 0.8
    third = (2.0*B2-8.0*(1.0+0.5*b_rho*modify)**4/(math.pi*modify**2))*k_n*fi_kn
    r_0 = 1.0+math.sqrt(math.pi/6.0)*modify/(1.0+0.5*b_rho*modify**2*(first+second+third))
    r = 1.0/r_0


    g[4,:,ly] = g[3,:,ly]

    g[8,:,ly] = (r*g[7,:,ly] + (1-r)*(g[6,:,ly]))

    g[6,:,ly] = (r*g[5,:,ly] + (1-r)*(g[8,:,ly]))

### ************************************************

### Slip bottom wall velocity b.c.
@njit(cache=True,fastmath=True,nogil=True)
def nb_slip_bottom_wall_velocity(lx, ly, g, N, k_n, fi_kn, modify, b_rho):
    first = (1.0/N)**2/(4.0*k_n*fi_kn)
    sigma = 1.0
    sigma_v = (2.0-sigma)/sigma
    B1 = (1.0-0.1817*sigma)
    ...
    g[7,:,0] = (r*g[8,:,0] + (1-r)*(g[6,:,0]))

    g[5,:,0] = (r*g[6,:,0] + (1-r)*(g[8,:,0]))