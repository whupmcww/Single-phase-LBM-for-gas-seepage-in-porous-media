...
@njit(parallel=True,cache=True,fastmath=True,nogil=True)
def nb_col_str(g, g_eq, g_up, om_p, om_m, om, c, ns, nx, ny, lx, ly, f):

    # Take care of q=0 first
    g_up[0,:,:] = (1.0-om_p)*g[0,:,:] + om_p*g_eq[0,:,:] + f[0,:,:]
    g   [0,:,:] = g_up[0,:,:]
...
### ************************************************
### Zou-He left wall pressure b.c.
@njit(cache=True,fastmath=True,nogil=True)
def nb_zou_he_left_wall_pressure(lx, ly, u, rho_left, u_left, rho, g):
...
### periodic left b.c.
@njit(cache=True,fastmath=True,nogil=True)
def nb_periodic_left(lx, ly, u, rho, g):
    u[0,0,:] = u[0,-1,:]
    u[1,0,:] = u[1,-1,:]
...
### periodic right b.c.
@njit(cache=True,fastmath=True,nogil=True)
def nb_periodic_right(lx, ly, u, rho, g):
    u[0,-1,:] = u[0,-2,:]
    u[1,-1,:] = u[1,-2,:]
...
### ************************************************
### New Zou-He left wall velocity b.c.
@njit(cache=True,fastmath=True,nogil=True)
def nb_zou_he_left_wall_velocity_new(lx, ly, u, u_left, rho, g, lattice):
    ...
    for j in range(ly):
        if lattice[0,j] == 0:
            ...
            rho[0,j] = (g[0,0,j] + g[3,0,j] + g[4,0,j] +
                        2.0*g[2,0,j] + 2.0*g[6,0,j] +
                        2.0*g[7,0,j] )/(1.0 - u[0,0,j])

            g[1,0,j] = (g[2,0,j] + cst1*rho[0,j]*u[0,0,j])

            g[5,0,j] = (g[6,0,j] - cst3*(g[3,0,j] - g[4,0,j]) +
                        cst2*rho[0,j]*u[0,0,j] +
                        cst3*rho[0,j]*u[1,0,j] )

            g[8,0,j] = (g[7,0,j] + cst3*(g[3,0,j] - g[4,0,j]) +
                        cst2*rho[0,j]*u[0,0,j] -
                        cst3*rho[0,j]*u[1,0,j] )
        else:
            u[0,0,j] = 0.0 
...
@njit(parallel=True,cache=True,nogil=True)
def nb_bounce_back_obstacle_ww(boundary, ns, sc,
                            g_up, g, u, lattice):
...
def nb_bounce_back_obstacle_kn_ww(boundary, g, g_up, r):
    ...
    for k in nb.prange(len(boundary)):
        ...
        if q == 2:
            g[1,i,j]=g_up[2,i-1,j]
            g[5,i,j]= (r*g_up[6,i-1,j] + (1-r)*(g_up[7,i-1,j]))
            g[8,i,j]= (r*g_up[7,i-1,j] + (1-r)*(g_up[6,i-1,j]))

        if q == 3:
            g[4,i,j]=g_up[3,i,j+1]
            g[6,i,j]= (r*g_up[5,i,j+1] + (1-r)*(g_up[7,i,j+1]))
            g[8,i,j]= (r*g_up[7,i,j+1] + (1-r)*(g_up[5,i,j+1]))

        if q == 4:
            g[3,i,j]=g_up[4,i,j-1]
            g[5,i,j]= (r*g_up[6,i,j-1] + (1-r)*(g_up[8,i,j-1]))
            g[7,i,j]= (r*g_up[8,i,j-1] + (1-r)*(g_up[6,i,j-1]))

        if q == 5:
            g[6,i,j] = g_up[5,i+1,j+1]
            # g[2,i,j] = (r*g_up[1,i+1,j+1] + (1-r)*(g_up[3,i+1,j+1]))
            # g[4,i,j] = (r*g_up[3,i+1,j+1] + (1-r)*(g_up[1,i+1,j+1]))

        
        if q == 6:
            g[5,i,j] = g_up[6,i-1,j-1]
            # g[1,i,j] = (r*g_up[2,i-1,j-1] + (1-r)*(g_up[4,i-1,j-1]))
            # g[3,i,j] = (r*g_up[4,i-1,j-1] + (1-r)*(g_up[2,i-1,j-1]))


        if q == 7:
            g[8,i,j] = g_up[7,i-1,j+1]
            # g[1,i,j] = (r*g_up[2,i-1,j+1] + (1-r)*(g_up[3,i-1,j+1]))
            # g[4,i,j] = (r*g_up[3,i-1,j+1] + (1-r)*(g_up[2,i-1,j+1]))


        if q == 8:
            g[7,i,j] = g_up[8,i+1,j-1]
            # g[2,i,j] = (r*g_up[1,i+1,j-1] + (1-r)*(g_up[4,i+1,j-1]))
            # g[3,i,j] = (r*g_up[4,i+1,j-1] + (1-r)*(g_up[1,i+1,j-1]))