...
class array(base_app):
    def __init__(self):
        ...
        #                    'square', 4.0*self.dx, pos)
        #     self.obstacles.append(obs)
        filename = 'G:/LBM/Poiseuille flow/sample/' + 'yuan.txt'
        ...
        for i in range(self.nx):
            for j in range(self.ny):
                if in_dat[i,j] != 0:
                    ii = i/self.L_lbm*self.x_max + 0.5*self.x_max/self.L_lbm
                    jj = j/self.L_lbm*self.x_max - 0.5*self.x_max/self.L_lbm
                    obs = obstacle('array', 4, 100,
                            'square', self.x_max/self.L_lbm, (ii, jj)) 
                    self.obstacles.append(obs)
                    self.n_obs += 1
                    #print(self.n_obs)
        obs = obstacle('array', 4, 100,
                            'square', self.x_max/self.L_lbm, (25.0*self.x_max/self.L_lbm-0.5*self.x_max/self.L_lbm, 50.0**self.x_max/self.L_lbm+0.5*self.x_max/self.L_lbm))
        self.obstacles.append(obs)
        self.n_obs += 1
    ...
    def initialize(self, lattice):
        ...
        # filename = 'G:/LBM/Poiseuille flow/sample/' + '2D_200_200.txt'
        # lattice.add_obstacle_1(filename)
