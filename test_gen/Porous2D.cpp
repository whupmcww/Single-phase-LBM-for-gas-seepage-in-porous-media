...
Porous2D::Porous2D(int nx, int ny, double por = 0.05, double p = 0.05, double p_zh = 0.05, double p_zv = 0.05,
	double p_f = 0.0125):NX(nx), NY(ny), phi(por), p_cd(p), z_h(p_zh), z_v(p_zv), f(p_f)
{
}
...
void Porous2D::output2datatxt(const int Total_M, const int Total_N, int* S, string filename)
{
	ofstream outfile;
	string title = "2D porous media";
	outfile.open(filename);
	...
	outfile.close();
	std::cout << "Output completed for txt." << endl;
}
