...
class Porous2D
{
    public:
		...
		const double z_h;     /// Probability in orthogonal direction
		const double z_v;     /// Probability in orthogonal direction
        const double f;   /// Probability in oblique direction

        /// Constructor
        Porous2D(int nx, int ny, double por, double p, double p_zh, double p_zv, double p_f);
		~Porous2D();
		...
		void output2datatxt(const int Total_M, const int Total_N, int* S, std::string filename);
